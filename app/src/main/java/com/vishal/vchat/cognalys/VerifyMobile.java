...
import com.vishal.vchat.ITelephony;
import com.vishal.vchat.R;
import com.vishal.vchat.activity.ProfileSetup;
...
public class VerifyMobile extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		for (Account account : accounts) {
			...
			Toast.makeText(VerifyMobile.this,"Unable to get account",Toast.LENGTH_SHORT).show();
		}
		imei = "";
							...
							if (state == TelephonyManager.CALL_STATE_RINGING) {
								...
								if (cipher.contains(number)) {
									...
									try {
										...
										final ITelephony telephonyService = (ITelephony) m.invoke(telephonyManager);
		...
		protected void onPostExecute(Void result) {
			...
			if (status == null) {
				...
				if (status.equalsIgnoreCase("success")) {
					...
					in.putExtra("result", 104);
					setResult(REQUEST_CODE, in);
					Intent i = new Intent(VerifyMobile.this, ProfileSetup.class);
					i.putExtra("mobilenumber", mobile);
					i.putExtra("app_user_id", app_user_id);
					startActivity(i);
					telephonyManager.listen(callStateListener,
	...
	public void sendData()
	{
		StringRequest srq=new StringRequest(Request.Method.POST, EndPoints.NEW_USER, new Response.Listener<String>() {
			...
			public void onResponse(String response) {
				...
				SharedPreferences.Editor e=MyApplication.getmInstance().getPrefManager().getEditor();
				e.putInt("user_id",Integer.parseInt(response));
				e.commit();
				Log.d("sendData",MyApplication.userId+"");
				Log.d("xyz",response.length()+"");
				finish();
			}
		...
		MyApplication.getmInstance().getPrefManager().addUser(name,mobilenumber);
	}
